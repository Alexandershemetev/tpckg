...
The `tpckg` package has data from NOAA Significant Earthquakes Database, helper functions to clean and filter the data,
...
recent_earthquakes <- clean_earthquakes %>%
    filter(COUNTRY == "CHINA", DATE >= ymd('2005-01-01'))#'
...
recent_earthquakes <- clean_earthquakes %>%
    filter(COUNTRY == "CHINA" | COUNTRY == "USA", DATE >= ymd('2005-01-01'))
g <- ggplot(recent_earthquakes,
            aes(x = DATE, y = COUNTRY, size = EQ_PRIMARY, color = TOTAL_DEATHS))
g <- g + geom_timeline(alpha = 0.5)
...
```{r, fig.width=7, fig.height=7}
clean_earthquakes %>%
    filter(COUNTRY == "JAPAN" & lubridate::year(DATE) >= 2005) %>%
